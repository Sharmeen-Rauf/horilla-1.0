...
def _build_faq_context():
    """Build FAQ context string for AI."""
    faqs = FAQ.objects.all()[:50]
    if not faqs:
        return "No FAQs in the database yet."
    lines = []
    for faq in faqs:
...
def _build_retrieved_context(query: str) -> str:
    """
    Retrieve relevant FAQ chunks from the persisted RAG index.
    Falls back to the full FAQ context if Chroma/index is unavailable.
    """
    try:
        from helpdesk.rag_faq import retrieve_faq_chunks

        chunks = retrieve_faq_chunks(query, k=4)
        if not chunks:
            return _build_faq_context()
        blocks = []
        for c in chunks:
            blocks.append(f"Q: {c.question}\nA: {c.answer_chunk}")
        return "\n\n".join(blocks)
    except Exception:
        return _build_faq_context()
...
def ai_assistant_view(request):
    """Render the AI Helpdesk Assistant chat page."""
    return render(request, "helpdesk/ai_assistant.html")


@login_required
@require_http_methods(["POST"])
def ai_assistant_chat(request):
    """Handle chat message and return AI response (JSON)."""
    try:
    ...
    faq_context = _build_retrieved_context(user_message)
    system_prompt = (
        "You are a helpful HR helpdesk assistant for WorkAura. "
        "Answer employee questions about policies, leave, attendance, tickets, and general HR topics. "
        "Use the following FAQ knowledge when relevant. If the answer is not in the FAQs, "
        "give a helpful general response. Be concise and professional.\n\n"
        f"FAQ Knowledge:\n{faq_context}"
    )
    ...
    response_text = chat(messages_list)
